...
exponential backoff. If CANARD lists an item but its detail remains unavailable,
the updater keeps its last known detail from `canard.json`. Set another retry
count with `--item-retries`, for example
